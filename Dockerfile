...
ENV PINPOINTING_JAR pinpointing-experiments-0.1.1-SNAPSHOT-93b216b-jar-with-dependencies.jar
...
CMD java -cp $PINPOINTING_JAR org.liveontologies.pinpointing.ExperimentServer --ontologies ontologies.tar.gz --results results.tar.gz \
	./scripts/run_ELK-SAT_experiments.sh '<t>' '<g>' ontologies.tar.gz experiments docker $PINPOINTING_JAR scripts workspace results.tar.gz
